...
def loss_weight_dw(bbox_cls_scores, gt_labels,
                        center_prior_weights, ious, inside_gt_bbox_mask, mask_positive):
    """
    return [B, L]
    """
    ...
    if num_gts > 0:
        ...
        p_neg_weight = normalize(ious * mask_positive).min(axis=1) * inside_gt_bbox_mask.max(axis=1)
    ...
    p_pos_weight = (paddle.exp(5*p_pos) * p_pos * center_prior_weights) / (paddle.exp(3*p_pos) * p_pos * center_prior_weights).sum(0, keepdim=True).clip(min=1e-12)
...
class TaskAlignedAssigner(nn.Layer):
    ...
    def forward(self,
        ...
        #debug compute dist between gt and pred bbox, [B, n, L]
